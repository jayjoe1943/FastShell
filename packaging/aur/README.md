...
- 本地生成一把专用 SSH key（**不要**复用日常 key）：
  ```sh
  ssh-keygen -t ed25519 -f ~/.ssh/aur -C "FastShell-aur"
  ```
...
### 3. 首次手动创建 AUR 包仓库
AUR 上必须先存在 `FastShell-bin` 这个包，工作流才能 push 更新。第一次手动建：
```sh
git clone ssh://aur@aur.archlinux.org/FastShell-bin.git
cd FastShell-bin
# 把本仓库的 packaging/aur/PKGBUILD 复制进来，按需把 pkgver 改成最新 release 版本
cp /path/to/FastShell/packaging/aur/PKGBUILD .
...
git commit -m "Initial import: FastShell-bin"
git push
```
...
## 备注
- 包名用 `FastShell-bin`（二进制包惯例）。若以后想要从源码编译的 `FastShell`
