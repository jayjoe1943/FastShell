#!/usr/bin/env bash
#
# Install FastShell's icon + desktop entry on Linux so the GNOME/Ubuntu dock and
...
# running window to its .desktop file via the window's app_id — FastShell sets
# that to "FastShell" (slint::set_xdg_app_id), and this script's StartupWMClass
# matches it.
#
# Usage:
#   ./install-linux.sh [/path/to/FastShell-binary]
# You normally don't need an argument: when run from inside a release package
# (the `FastShell` binary sits next to this script) it is picked up automatically.
# In the source tree it falls back to ./target/release/FastShell.
...
if [ -n "${1:-}" ]; then
    BIN="$1"
elif [ -x "$SCRIPT_DIR/FastShell" ]; then
    BIN="$SCRIPT_DIR/FastShell"
else
    BIN="$SCRIPT_DIR/../target/release/FastShell"
fi
...
if [ ! -x "$BIN" ]; then
    echo "error: FastShell binary not found: $BIN" >&2
    echo "Run this script from the extracted release folder (it sits next to the" >&2
    echo "'FastShell' binary), or pass the binary path as an argument." >&2
    exit 1
fi
...
StartupNotify=true
StartupWMClass=FastShell
EOF
chmod 644 "$APP_DIR/FastShell.desktop"
...
echo "Installed:"
echo "  icon    -> $ICON_DIR/FastShell.png"
echo "  desktop -> $APP_DIR/FastShell.desktop"
echo "  exec    -> $BIN"
echo
